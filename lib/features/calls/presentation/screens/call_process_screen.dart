...
class _CallProcessScreenState extends State<CallProcessScreen> {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            const Text('Send WhatsApp / إرسال واتساب',
  ...
  Future<void> _sendWhatsApp(
      String phoneNumber, String? name, String rawMessage) async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            const Text('Great Work! / عمل رائع',
                style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            const Text('You have finished this list.',
                style: TextStyle(color: Colors.grey)),
            const Divider(height: 32),
            _buildStatRow(Icons.check_circle_rounded, 'Answered:',
                '$_statsAnswered', Colors.green),
            const SizedBox(height: 12),
            _buildStatRow(
                Icons.cancel_rounded, 'Missed:', '$_statsMissed', Colors.red),
            const SizedBox(height: 12),
            _buildStatRow(Icons.chat_rounded, 'WhatsApp:', '$_statsWhatsapp',
                Colors.teal),
    ...
    if (pickedDate != null && mounted) {
      ...
      if (pickedTime != null) {
        ...
        await NotificationService().scheduleLocalNotification(
          ...
          body:
              'Call ${currentItem.name ?? "Unknown"} (${currentItem.phone}) now!',
        ...
        child: Column(
          children: [
            Container(
              ...
              decoration: BoxDecoration(color: theme.cardTheme.color, borderRadius: BorderRadius.circular(28), border: Border.all(color: Colors.black.withValues(alpha: 0.05))),
              child: Column(
                children: [
                  CircleAvatar(
                      radius: 40,
                      backgroundColor: theme.colorScheme.primary,
                      child: Text(currentItem.name?[0] ?? '?',
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
              _buildBtn(Icons.call_rounded, 'Call', Colors.blue,
                  () => _makeCall(currentItem.phone)),
              _buildBtn(
                  Icons.chat_rounded,
                  'WhatsApp',
                  const Color(0xFF128C7E),
                  () => _showWhatsAppOptions(
                      currentItem.phone, currentItem.name)),
            ]),
            const SizedBox(height: 32),
            TextField(
  ...
  Widget _buildBtn(
    ...
    return Column(children: [
      InkWell(onTap: onTap, borderRadius: BorderRadius.circular(50), child: Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(color: color.withValues(alpha: 0.1), shape: BoxShape.circle), child: Icon(icon, color: color, size: 28))),
      const SizedBox(height: 8),
      Text(label,
          style: TextStyle(
